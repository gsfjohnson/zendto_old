...
# configured after ZendTo itself has been installed.
exit 0
...
configure_postfix() {
  ...
  if [ "$OSVER" -le "6" ]; then
    if chkconfig --list postfix 2>/dev/null | grep -q 3:on; then
    ...
    # Systemd on CentOS / RHEL 7 and above
...
if rpm -q --quiet postfix; then
  ...
  configure_postfix
  DONE=yes
elif rpm -q --quiet sendmail; then
  ...
  if yesno "for you now" "y"; then
    ...
    if [ "$OSVER" -le "6" ]; then
      ...
      chkconfig exim off >/dev/null 2>&1

...
class NSSDropoff {
  ...
  )
  {
    ...
    if ( $this->_okayForDownloads ) {
      ...
      if ( $fileList && count($fileList) ) {
        ...
        header("Content-type: " . htmlspecialchars($fileList[0]['mimeType']));
        ...
        if (preg_match("/MSIE/", $_SERVER["HTTP_USER_AGENT"])) {
          header('Content-Disposition: attachment; filename="' .
                 rawurlencode($fileList[0]['basename']) . '"');
        } else {
          header('Content-Disposition: attachment; filename="' .
                 $fileList[0]['basename'] . '"');
        }
  ...
  */
  public function HTMLWrite()
  {
    ...
    // Bug: $smarty->assign('maxBytesForFile', NSSFormattedMemSize($this->_maxBytesForFile));
    // Bug: $smarty->assign('maxBytesForDropoff', NSSFormattedMemSize($this->_maxBytesForDropoff));
    // Bug: $smarty->assign('retainDays', $this->_retainDays);
  ...
  */
  ...
  )
  {
    ...
    $smarty->assign('isSendable', ( $clickable && ( $isAdmin || $isSender )));

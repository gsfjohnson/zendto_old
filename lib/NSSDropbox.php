  ...
  */
  ...
  )
  {
	...
	NSSError("Could create ZendTo database handle");
      ...
      if ( $intValue = intval( $prefs['warnDaysBeforeDeletion'] ) ) {
...
public function updateAddressbook( $recips ) {
  ...
  */
  ...
  /*!
    @function recaptchaLanguage
    
    Accessor pair for getting/setting the recaptchaLanguage.
  */
  ...
  /*!
    @function recaptchaInvisible
    
    Accessor pair for getting/setting the recaptchaInvisible.
  */
  ...
  */
  public function isLocalIP()
  {
    foreach ($this->localIPSubnets() as $subnet) {
      ...
      $subnet = '/^'.$subnet.'/';
      // $this->writeToLog("Comparing ".getClientIP()." with ".$subnet);
      if (preg_match($subnet, getClientIP())) {
        return TRUE;
      }
    }
    return FALSE;
  ...
  */
  ...
  )
  {
    ...
    //  Add to database:
    // JKF $filename, $claimID shouldn't be here, bug.
